...
<template>
  <div class="checkin-wrapper">
    <!-- Steps indicator -->
    <div class="steps-bar">
      <div class="step-item" :class="{ active: currentStep === 0, done: gpsDone }">
        <span class="step-num">1</span>
        <span class="step-label">GPS</span>
      </div>
      <div class="step-line" :class="{ done: gpsDone }"></div>
      <div class="step-item" :class="{ active: currentStep === 1, done: faceDone }">
    ...
    <div v-if="currentStep === 1" class="step-card glass-card">
      ...
      <button v-if="cameraReady && !faceDone" class="capture-btn cursor-pointer" @click="captureFace">
...
:root.dark .step-num { background: rgba(30, 41, 59, 0.6); border-color: rgba(255, 255, 255, 0.1); }
...
:root.dark .step-item.active .step-label { color: #60A5FA; }
...
:root.dark .step-line { background: rgba(255, 255, 255, 0.1); }
.step-line.done { background: #059669; }

/* Card */
.step-card { padding: 32px 28px; text-align: center; }
.step-icon { margin-bottom: 12px; }
.step-title { font-size: 1.25rem; font-weight: 700; color: #1E293B; margin: 0 0 6px; }
:root.dark .step-title { color: #F1F5F9; }
.step-desc { font-size: 0.875rem; color: #94A3B8; margin: 0 0 20px; }

/* Loading */
.loading-row, .loading-state { display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 20px 0; color: #64748B; }
...
/* Error */
.error-box { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 16px; background: #FEF2F2; border: 1px solid #FECACA; border-radius: 12px; color: #DC2626; font-size: 0.875rem; }
:root.dark .error-box { background: rgba(220, 38, 38, 0.1); border-color: rgba(220, 38, 38, 0.2); }
...
.coord-box { display: flex; flex-direction: column; gap: 3px; font-size: 0.8125rem; color: #64748B; background: #F8FAFC; border-radius: 10px; padding: 12px 20px; }
:root.dark .coord-box { background: rgba(30, 41, 59, 0.5); color: #94A3B8; }
...
:root.dark .face-img { border-color: rgba(255, 255, 255, 0.1); }
...
:root.dark .summary-row { background: rgba(30, 41, 59, 0.5); }
.summary-label { color: #64748B; }
.summary-value { color: #1E293B; font-weight: 500; }
:root.dark .summary-value { color: #E2E8F0; }
...
.detail-row { display: flex; justify-content: space-between; padding: 12px 16px; background: #F8FAFC; border-radius: 10px; font-size: 0.875rem; color: #64748B; }
:root.dark .detail-row { background: rgba(30, 41, 59, 0.5); }
.text-success { color: #059669; font-weight: 600; }
.text-error { color: #DC2626; font-weight: 600; }

/* Buttons */
.btn-primary { padding: 10px 24px; background: linear-gradient(135deg, #2563EB, #3B82F6); color: white; border: none; border-radius: 10px; font-size: 0.9rem; font-weight: 600; transition: all 0.2s; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.3); min-height: 44px; font-family: inherit; }
.btn-primary:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(37, 99, 235, 0.4); }
.btn-ghost { padding: 10px 24px; background: #F1F5F9; color: #64748B; border: 1px solid #E2E8F0; border-radius: 10px; font-size: 0.9rem; font-weight: 500; transition: all 0.2s; min-height: 44px; font-family: inherit; }
:root.dark .btn-ghost { background: rgba(30, 41, 59, 0.5); border-color: rgba(255, 255, 255, 0.1); color: #94A3B8; }
